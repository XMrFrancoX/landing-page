<script lang="ts">
  ...
  async function requestBaja(requestId: string) {
    if (!confirm('¿Confirmás que querés solicitar la baja de este servicio? Le avisamos a NMF para que se contacte con vos.')) return;
          ...
          <div class="group rounded-2xl border bg-white shadow-sm transition-all hover:shadow-md hover:border-primary/20">
            ...
            <div class="px-5 py-4 border-t bg-slate-50/50">
              <div class="flex items-center gap-0">
                ...
                <!-- Step 2 -->
                <div class="flex flex-col items-center gap-1.5 flex-shrink-0">
                  <div class="h-3 w-3 rounded-full {req.status === 'contacted' || req.status === 'resolved' || req.status === 'baja_solicitada' ? 'bg-primary ring-4 ring-primary/10' : 'bg-slate-200'}"></div>
                  <span class="text-[10px] font-medium {req.status === 'contacted' || req.status === 'resolved' || req.status === 'baja_solicitada' ? 'text-slate-900' : 'text-slate-400'} whitespace-nowrap">Contactado</span>
                </div>
                <!-- Line -->
                <div class="flex-1 h-0.5 mx-1 rounded {req.status === 'resolved' || req.status === 'baja_solicitada' ? 'bg-primary' : 'bg-slate-200'}"></div>
                <!-- Step 3 -->
                <div class="flex flex-col items-center gap-1.5 flex-shrink-0">
                  <div class="h-3 w-3 rounded-full {req.status === 'resolved' || req.status === 'baja_solicitada' ? 'bg-green-500 ring-4 ring-green-500/10' : 'bg-slate-200'}"></div>
                  <span class="text-[10px] font-medium {req.status === 'resolved' || req.status === 'baja_solicitada' ? 'text-slate-900' : 'text-slate-400'} whitespace-nowrap">Finalizado</span>
                </div>
              </div>
            </div>
            ...
            <div class="px-5 py-3 border-t flex items-center justify-between gap-2">
              <button
                ...
                class="inline-flex items-center gap-1.5 text-xs font-medium text-slate-600 hover:text-primary transition-colors"
              >
            ...
            {#if openThreadId === req.id}
              <div class="px-5 py-4 border-t bg-slate-50/50 space-y-3">
                {#if thread.length === 0}
                  ...
                  <div class="space-y-2 max-h-64 overflow-y-auto">
                    {#each thread as msg}
                      <div class="flex {msg.sender_role === 'nmf' ? 'justify-start' : 'justify-end'}">
                        <div class="max-w-[85%] rounded-lg px-3 py-2 text-xs {msg.sender_role === 'nmf' ? 'bg-white border text-slate-700' : 'bg-primary/10 text-slate-800'}">
